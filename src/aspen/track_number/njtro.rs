...
// Track numbers and consist data for NJT Rail Operations (NJTRO)

// Amtrak GTFS stop ID -> NJT penta station ID (same as GTFS stop_code)
const AMTRAK_TO_PENTA: &[(&str, &str)] = &[
    ("PHL", "PH"), // Philadelphia 30th St
    ("MET", "MP"), // Metropark
    ("NBK", "NB"), // New Brunswick
    ("EWR", "NA"), // Newark International Airport
    ("NWK", "NP"), // Newark Penn
    ("PJC", "PJ"), // Princeton Junction
    ("TRE", "TR"), // Trenton
    ("NYK", "NY"), // New York Penn
];
...
pub async fn fetch_njtro_track_data(
    chateau_id: &str,
    pool: &CatenaryPostgresPool,
) -> Option<NjtRoTrackData> {
    let is_amtrak = chateau_id == "amtrak";
    let is_njtro = chateau_id == "nj~transit~rail";

    // TODO also support Amtrak
    if !is_njtro {
        return None;
    }
    ...
    if is_amtrak {
        for &(amtrak_gtfs, penta_station_id) in AMTRAK_TO_PENTA {
            code_to_ids
                .entry(penta_station_id.to_string())
                .or_default()
    ...
    let resp_dvstations = match direct_client
